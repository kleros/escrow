...
export default ({ accounts, arbitrabletx }) => {
  ...
  switch (arbitrabletx.status) {
    case arbitrableTxConstants.NO_DISPUTE:
      if (arbitrabletx.amount === '0')
        ...
        Date.now() - arbitrabletx.lastInteraction * 1000 >=
          arbitrabletx.timeoutPayment * 1000 &&
        accounts[0] === arbitrabletx.sender
      )
    ...
    case arbitrableTxConstants.WAITING_SENDER:
      if (!isFeePaid(arbitrabletx))
        ...
        timeout(arbitrabletx) === 0 &&
        accounts[0] === arbitrabletx.receiver
      )
    ...
    case arbitrableTxConstants.DISPUTE_RESOLVED:
      if (arbitrabletx.ruling === null)
        ...
        if (
          arbitrabletx.ruling === '1' ||
          (arbitrabletx.ruling === '0' && accounts[0] === arbitrabletx.receiver)
        )
          // eslint-disable-next-line no-mixed-operators
          return statusArbitrableTxConstants.APPEALABLE_WINNER
      // eslint-disable-next-line no-mixed-operators
      if (
        arbitrabletx.ruling === '2' ||
        (arbitrabletx.ruling === '0' && accounts[0] === arbitrabletx.receiver)
      )
...
const timeout = arbitrabletx =>
  (Number(arbitrabletx.lastInteraction) + Number(arbitrabletx.feeTimeout)) *
    1000 -
    Date.now() >
  0
